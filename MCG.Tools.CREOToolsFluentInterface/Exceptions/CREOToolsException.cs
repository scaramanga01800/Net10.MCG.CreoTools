...
{
    class CREOToolsException : McgCommonLibException
    {
        public CREOToolsException(string message) : base(message)
        {
            TraceLog.AddTraceLog(Message);
        }

        public CREOToolsException(string CurrentClass = "UnknownClass", Exception CurrentException = null, [CallerMemberName] string CurrentMethod = "UnknownMethod") : base($"Exception in {CurrentClass}.{CurrentMethod} : {CurrentException.Message}")
        {
            TraceLog.AddTraceLog(Message);
        }
        ...
        public new static void SendMessageBox(string CurrentClass = "UnknownClass", Exception CurrentException = null, [CallerMemberName] string CurrentMethod = "UnknownMethod")
        {
            ...
            {
                throw new Exception($"Exception in CREOToolsException.CREOToolsException : {ex.Message}");
            }
        }
    }
}

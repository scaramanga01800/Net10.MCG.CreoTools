...
namespace MCG.CREO_Tools.QuickSearch.Exceptions
{
    public class QuickSearchException : McgCommonLibException
    {
        public QuickSearchException(string message) : base(message)
        {
            TraceLog.AddTraceLog(Message);
        }

        public QuickSearchException(string CurrentClass = "UnknownClass", Exception CurrentException = null, [CallerMemberName] string CurrentMethod = "UnknownMethod") : base($"Exception in {CurrentClass}.{CurrentMethod} : {CurrentException.Message}")
        {
            TraceLog.AddTraceLog(Message);
        }
        ...
        public new static void SendMessageBox(string CurrentClass = "UnknownClass", Exception CurrentException = null, [CallerMemberName] string CurrentMethod = "UnknownMethod")
        {
            ...
            {
                throw new Exception($"Exception in QuickSearchException.SendMessageBox : {ex.Message}");
            }
        }
    }
}
